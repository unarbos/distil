...
lines.append(
    ...
    "``distil/eval/service.py`` defines ``ROUND_INTERVAL_S = 4200`` and "
    "after each completed round the loop runs ``for _ in range(ROUND_"
    "INTERVAL_S): time.sleep(1)``. So if the validator journal is quiet, "
    "``current_round.completed == True``, pod GPUs are idle, and "
    "``last_eval_block`` was within the last ~70 min — that is **between "
    "rounds**, not a hang. The bot must NOT respond \"eval is stuck\" / "
    "\"pod orchestrator dead\" / \"restart the orchestrator\" in this "
    "case. The only real stuck-mid-round signal is: ``api/health.eval_"
    "active=true`` AND ``Pod Live Snapshot.phase != finished`` AND the "
    "round has been alive for >>round budget (~90 min). Anything else "
